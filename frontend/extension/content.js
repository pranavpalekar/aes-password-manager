...
// Function to detect and click login button
...
function fillLoginForm(username, password, autoLogin = false) {
  ...
  if (loginForms.length > 0) {
    ...
    // If autoLogin is enabled, try to click the login button
    if (autoLogin) {
      setTimeout(() => {
        findAndClickLoginButton();
      }, 100); // Small delay to ensure form is filled
    } else {
      // Just highlight the login button if available
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'detectForms') {
    ...
    const success = fillLoginForm(message.username, message.password, message.autoLogin);

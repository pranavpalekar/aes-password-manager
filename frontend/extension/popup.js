...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function displayPasswords(passwords) {
    ...
    passwords.forEach(pwd => {
      ...
      autoLoginCheckbox.id = `auto-login-${pwd.id}`;
      
      const autoLoginLabel = document.createElement('label');
      autoLoginLabel.htmlFor = `auto-login-${pwd.id}`;
      autoLoginLabel.textContent = 'Auto-login after filling';
      
      autoLoginContainer.appendChild(autoLoginCheckbox);
      autoLoginContainer.appendChild(autoLoginLabel);
      
      fillBtn.addEventListener('click', () => fillForm(pwd.username, pwd.password, autoLoginCheckbox.checked));
      ...
      item.appendChild(autoLoginContainer);
  ...
  function fillForm(username, password, autoLogin = false) {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
      ...
      chrome.runtime.sendMessage({ action: 'ensureContentScriptLoaded' }, async (response) => {
        ...
        chrome.tabs.sendMessage(tabs[0].id, {
          ...
          if (response && response.success) {
            if (autoLogin) {
              showStatus('Form filled and login attempted!', 'success');
            } else {
